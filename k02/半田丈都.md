...
- プログラミング書法鉄則**「関数定義の中に生の数字を書かない」**
  - main関数の定義の中に
    ...
    sigmaB=5.5; 
    ```
    という行がありますが，これらの生の数字は関数定義の中に書かないでください．では，どうするかというと，関数の外のヘッダに生の数字を書くのです．
    ```
    #define _AVE_A_ 170.8
    #define _SIGMA_A_ 5.43
    #define _AVE_B_ 169.7
    #define _SIGMA_B_ 5.5
    ```
    を`#include`あたりの`#`行のあとに書きます．`#define`は「文字列の別名定義」でして，"170.8"という5文字に"_AVE_A_"という7文字の別名を定義しています．
    "_AVE_A_"という別名を定義していますが，この名前は任意です．

    この「数字の別名」を関数の中で，数字の代わりに書きます．
    ```
    aveA = _AVE_A_;
    ```
    のような感じです．なぜこうするかは，[../Reference/Coding.tips]に書きましたが，端的にいうと，
    **生の数字は意味不明なので，意味を分かるようにする**ということです．
- インデント（字下げ）を合わせてください．while文の中のインデントは，直後のsscanfと同じインデントに並ぶはずです．他の箇所にもインデントが揃っていない場所があります．インデントは大事です．
