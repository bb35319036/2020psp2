...
７．平均を求める関数を自作するため資料より、平均を求める式を入力
12．分散を求める関数を自作するため資料より、分散を求める式を入力
24.25 読み取るファイルを入力させ、記憶する
29　ファイルを開く
303132　ファイルが開けない場合失敗とし終了する
34　表の1行目に数字がないため2行目から読み取らせるための操作（上手なやり方が考え付きませんでした、、）
35～42.表の数字を読み取り数字がなくなるまで平均と分散の計算を繰り返す。
52.資料より不偏分散の式を入力し計算する。
54.55.57.58.平均、分散、平均の推定値、分散の推定値を出力する。

## 修正履歴

[コメント1]
- 男性のみのデータ，heights_male.csvを読んでください．これなら1行目から数字があります．
- 出力するのは，「標本平均」「標本分散」「母集団平均（推定値）」「母集団分散（推定値）」の4つです．3つではありません．
- コンパイルは`gcc -o k01 k01.c -W -Wall`などと，`-W`と`-Wall`の2つのオプションを加えて行ってみてください．すると，警告(Warning)がたくさん出ると思います．これらの警告が全く出ない状態にして提出してください．
  - unused variable 'hoge':'hoge'という変数は一度も使われてない無駄な変数です．
  - 'hoge' may be used uninitialized in this function: 'hoge'はこの関数の中で初期化されずに使われています．whileループに入らなかったりしたときに，printfなどでこの変数の値を表示させようとしたりしてませんか． 
